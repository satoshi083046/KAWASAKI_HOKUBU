...
'	室温 20℃ 湿度 20％
'
'	浸漬Aタンク 36/36   浸漬Bタンク 31/36  総計 67/72  で供給 米なし 残る5釜は空 送米      
'
'
'===================================================================================================================
select case 客先コード
...
case 川崎北部
	...
	if 浸漬タンクA水位>3900 then
		浸漬タンクA水位=0
	end if
	
	if 浸漬タンクB水位>3900 then
		浸漬タンクB水位=0
	end if
...
'タイマー処理	
'--------------------------------
tmr(not(FLOS3_LMT121),timdly_FLOS03_LMT121,20) '10 -> 20に変更
tmr(FLOS3_LMT121,timnotdly_FLOS03_LMT121,20) '10 -> 20に変更
if timdly_FLOS03_LMT121 or 浸漬タンクA米有り then
	浸漬タンクA米有り=ON
end if
if (浸漬タンクA米有り and timnotdly_FLOS03_LMT121) or (前処理工程AutoRun=OFF and 計量step=0 and 洗米step=0 and timnotdly_FLOS03_LMT121) then
	'DM_浸漬タンクA貯米釜数=0 'コメント化
	if not(運転準備完了) then
				DM_Aタンク送米現在値=0
	end if
	浸漬タンクA米有り=OFF
...
tmr(not(FLOS4_LMT121),timdly_FLOS4_LMT121,20) '10 -> 20に変更
tmr(FLOS4_LMT121,timnotdly_FLOS4_LMT121,20) '10 -> 20に変更
if timdly_FLOS4_LMT121 or 浸漬タンクB米有り then
	浸漬タンクB米有り=ON
end if
if (浸漬タンクB米有り and timnotdly_FLOS4_LMT121) or (前処理工程AutoRun=OFF and 計量step=0 and 洗米step=0 and timnotdly_FLOS4_LMT121) then
...
if RLY_電源確認納米庫  then
	...
	if not(予約運転モード) and 前処理注水完了 and DM_Aタンク設定値>0  then
		if FLOS3_LMT121 and XSW9_送米切換A側 then
			浸漬タンクA停止許可=OFF
		else
			浸漬タンクA停止許可=ON
		end if
	...
	if not(予約運転モード) and 前処理注水完了 and DM_Bタンク設定値>0 then
		if FLOS4_LMT121 and XSW10_送米切換B側 then
			浸漬タンクB停止許可=OFF
		else
			浸漬タンクB停止許可=ON
		end if
	...
	tmr(FLOS3_LMT121,nottim_FLOS3_LMT121,20)
	tmr(FLOS4_LMT121,nottim_FLOS4_LMT121,20)

	if PB21_前処理停止 or SSW4_納米庫手動 or 前処理計量完了 then
		前処理工程AutoRun=OFF
		計量AutoRun=OFF
	...
	if ldp(PB19_即時運転) and not(PB20_予約運転) and SSW4_納米庫自動 and not(almSSW4_納米庫自動) and not(PB21_前処理停止) then 'and 前処理設定完了 then
		res(tim_RLY_レベルスイッチ)
	...
	if ldp(PB20_予約運転) and not(即時運転モード) and SSW4_納米庫自動 and not(almSSW4_納米庫自動) and not(PB19_即時運転) and not(PB21_前処理停止) and PB33_納米庫全停止 then 'and 前処理設定完了 then
		res(tim_RLY_レベルスイッチ)
		if not(前処理工程AutoRun) and  前処理工程step=0 and 計量step=0 and 洗米step=0 and (DM_Aタンク送米現在値<DM_Aタンク設定値 or DM_Bタンク送米現在値<DM_Bタンク設定値) then
			'完了フラグ=OFF
...
if 前処理工程EmgStep=0 then  
	...
	if 前処理工程AutoMode or 前処理工程step<>0 then
		...
		case 1000
			...
			if DM_Aタンク設定値<>0 and DM_Aタンク送米現在値<DM_Aタンク設定値 and 浸漬タンクA水位<3650 then  '浸漬タンクA
			...
			if DM_Bタンク設定値<>0 and DM_Bタンク送米現在値<DM_Bタンク設定値 and 浸漬タンクB水位<3650 then  '浸漬タンクB
		...
		case 1001
			...
			if mSV6_給水電磁弁A and 浸漬タンクA水位>3650 then
				mSV6_給水電磁弁A=OFF
			end if

			if mSV11_給水電磁弁B and 浸漬タンクB水位>3650 then
				mSV11_給水電磁弁B=OFF
			end if

			if not(mSV1_給水電磁弁) and not(mSV6_給水電磁弁A) and not(mSV11_給水電磁弁B) then
				前処理注水完了=ON			
			end if
