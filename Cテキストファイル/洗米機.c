...
if 洗米EmgStep=0 then  
	...
	'
		...
		case 42
			if not(洗米AutoRun) then
				...
				if 無洗米モード then
					TIM_洗米バルブAON_val=洗米バルブAON時間_無洗米 '200
					TIM_洗米バルブAOFF_val=洗米バルブAOFF時間_無洗米 '30
					TIM_洗米バルブBON_val=洗米バルブBON時間_無洗米 '0
					TIM_洗米バルブBOFF_val=洗米バルブBOFF時間_無洗米 '0
					TIM_洗米羽根ON_val=洗米羽根ON時間_無洗米 '0
					TIM_洗米羽根OFF_val=洗米羽根OFF時間_無洗米 '0
				else
					TIM_洗米バルブAON_val=洗米バルブAON時間_洗米 '600
					TIM_洗米バルブAOFF_val=洗米バルブAOFF時間_洗米 '30
					TIM_洗米バルブBON_val=洗米バルブBON時間_洗米 '600
					TIM_洗米バルブBOFF_val=洗米バルブBOFF時間_洗米 '30
					TIM_洗米羽根ON_val=洗米羽根ON時間_洗米 '600
					TIM_洗米羽根OFF_val=洗米羽根OFF時間_洗米 '1000
				end if
	...
	'  マニュアルスイッチ操作はタイムエラーを発生させない
		...
		if ldp(pnlPBSV12_送米切換バルブA側) then
				mSV12_送米切換バルブA側=ON
				mSV13_送米切換バルブB側=OFF
		else if ldp(pnlPBSV13_送米切換バルブB側) then
				mSV12_送米切換バルブA側=OFF
				mSV13_送米切換バルブB側=ON
		end if
