...
'張込モータは常に手動でON/OFF可能
	...
	if not(THR1_張込用昇降機モータ) and PB33_納米庫全停止 then
		if ldp(PB1_張込用昇降ON) or ldp(pnlPBMC1_張込用昇降機) then
			if mMC1_張込用昇降機 then
				mMC1_張込用昇降機=OFF
			else
				mMC1_張込用昇降機=ON
...
tmr(SV12_送米切換バルブA側 and not(XSW9_送米切換A側) and not(pnlPBXSW9_送米切換A側) ,tim_XSW9_送米切換A側,#100)
if ldp(tim_XSW9_送米切換A側.B) then
	almXSW9_送米切換A側=ON
	計量ErrStep=1
end if

tmr(SV13_送米切換バルブB側 and not(XSW10_送米切換B側) and not(pnlPBXSW10_送米切換B側), tim_XSW10_送米切換B側,#100)
if ldp(tim_XSW10_送米切換B側.B) then
	almXSW10_送米切換B側=ON
	計量ErrStep=1
end if
...
else if almPDL1_パドルセンサー上限A and ResetFlg then
	almPDL1_パドルセンサー上限A=OFF
end if

if ldp(timPDL3_パドルセンサー上限B) then
	almPDL3_パドルセンサー上限B=ON
else if almPDL3_パドルセンサー上限B and ResetFlg then
...
if 計量EmgStep=0 then  
	...
	'
		...
		case 3
			if tim_切換えバルブ切換え待ち.B then 'ダブルソレノイドOFFタイミング
				if  (not(送米タンク切換え) or mSV12_送米切換バルブA側) and XSW9_送米切換A側  then
					mSV12_送米切換バルブA側=OFF
					mSV13_送米切換バルブB側=OFF
					inc(計量step)
				else if  (送米タンク切換え or mSV13_送米切換バルブB側) and XSW10_送米切換B側 then
					mSV12_送米切換バルブA側=OFF
					mSV13_送米切換バルブB側=OFF
					inc(計量step)
				end if
		...
		'計量中
		case 7
			if not(前処理計量完了) or ワンサイクルflg then '前処理全体の浸漬タンクA～Bへの計量完了
				res(CNT_計量現在値)
				mMC3_計量機=ON
				inc(計量step)
			else
		...
		case 8
			if not(PB33_納米庫全停止) then
				...
				if CNT_計量現在値.B then '指定計量（カップ数）終了
					if XSW9_送米切換A側 or SV12_送米切換バルブA側 then
						if DM_Aタンク設定値>0 and DM_Aタンク送米現在値=0 then
							浸漬時間計測中=ON
						end if
						inc(DM_Aタンク送米現在値) '送米釜数
					else if XSW10_送米切換B側 or SV13_送米切換バルブB側 then
	...
	'  マニュアルスイッチ操作はタイムエラーを発生させない
		...
		'-----------------------------------
		' 昇降モータON/OFF
		if  ldp(PB3_排出用昇降ON) or ldp(pnlPBMC2_排出用昇降機)and 計量step=0 then
			if mMC2_排出用昇降機 then
				mMC2_排出用昇降機=OFF
		...
		'-----------------------------------
		' 計量step
		if (ldp(PB5_計量機ON) or ldp(pnlPBMC3_計量機)) and PB33_納米庫全停止 and mMC2_排出用昇降機 and 計量step=0 then
				ワンサイクルflg=ON
				計量step=1
		end if		
